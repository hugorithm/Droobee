...
export async function playSong(parsedUserCommand: CommandContext, arg: string) {
    const track = await Track.from(arg, parsedUserCommand.originalMessage, {
        async onStart() {
    ...
    const user = `${parsedUserCommand.originalMessage.member?.nickname} (${parsedUserCommand.originalMessage.author.tag})`; 
